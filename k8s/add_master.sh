...
echo "Для успешного добавления ноды предварительно добавте ее в k8s-vars.txt"
echo "и запустите make-balancer.sh чтобы балансировщик знал про новый мастер"
echo " "
echo "Для добавления мастер-ноды запустить скрипт с параметрами - IP-address Hostname"
echo "Например:      add_master.sh 192.168.223.1 new-master1"
...
##  Добавляем новую мастер-ноду в /etc/hosts на все хосты кластера
echo "Добавляем новую мастер-ноду в /etc/hosts на все хосты кластера"
for val in $(seq 0 $NALL);do
   NODE=$(echo "${ALLHOSTS[$val]}" | awk '{print $1}')
   echo " "
   echo "----------------------------------------------------"
...
##  Подготовка дополнительной мастера-ноды
echo "Подготовка дополнительной мастера-ноды"
NODE=$1
echo " "
echo "----------------------------------------------------"
...
echo " "
echo "Инициализация кластера на дополнительной мастер-ноде"
echo "Дополнительная мастер нода: "$NODE
...
ssh -i $SSHKEY -t $SSHUSER@$FIRSTMASTER "kubectl label node $2 node-role.kubernetes.io/master=master"
...
echo " "
echo "----------------------------------------------------"
echo "Дополнительная мастер нода добавлена" 
